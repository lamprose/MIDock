...
class MainHook : IXposedHookLoadPackage, IXposedHookInitPackageResources {
    ...
    private fun showSettingDialog() {
        findMethodByCondition("$MIUI_HOME_LAUNCHER_PACKAGENAME.settings.MiuiHomeSettings") { m ->
            ...
            dialogBuilder.apply {
                val radiusSeek =
                        SettingSeekBar.Builder(context, "Dock圆角", Utils.getData("DOCK_RADIUS", 20)).build()
                val heightSeek =
                        SettingSeekBar.Builder(context, "Dock高度", Utils.getData("DOCK_HEIGHT", 84)).build()
                val sideSeek =
            ...
            XposedHelpers.findAndHookMethod(
                    ...
                    object : XC_MethodHook() {
                        override fun beforeHookedMethod(param: MethodHookParam) {
                            ...
                            val _context = param.args[0] as Context
                            param.result =
                                    Utils.dip2px(Utils.getData("DOCK_BOTTOM", DOCK_BOTTOM))
                        }
                    })
        } catch (e: Exception) {
